...
const SeatChooser = ({ chosenDay, chosenSeat, updateSeat }) => {
  ...
  useEffect(() => {
    ...
    const socket = io(process.env.NODE_ENV === 'production' ? '' : `ws://${window.location.hostname}:8000`, { transports: ['websocket'] });
  ...
  const freeSeats = () => {
    return numberOfSeats - (seats.filter(item => item.day === chosenDay).length);
  }
  ...
  return (
    <div>
      ...
      { (requests['LOAD_SEATS'] && requests['LOAD_SEATS'].success) && <p className='seats__info'> Free seats: {freeSeats()} / {numberOfSeats}</p> }
      { (requests['LOAD_SEATS'] && requests['LOAD_SEATS'].pending) && <Progress animated color="primary" value={numberOfSeats} /> }
